...
- [x] **T-010. User entity + миграция** — `email` (unique), `passwordHash` (скрыт через `@Exclude()`), `role` (enum), `isEmailVerified`, `createdAt`/`updatedAt`. Миграция `CreateUsers` создаёт таблицу + расширение `uuid-ossp`. Проверено: `migration:run`/`migration:revert` на чистом Postgres, unique constraint на `email` реально работает. Три инфраструктурных бага найдены и исправлены по пути (хардкод имени в `migration:generate`/`migration:create`, двойной экспорт `DataSource`, отсутствие `uuid-ossp` в миграции) — детали в [IMPLEMENTATION-LOG.md](IMPLEMENTATION-LOG.md#t-010-user-entity--миграция). Полный контракт — [docs/tickets/T-010-011-registration.md](tickets/T-010-011-registration.md).
- [x] **T-011. Регистрация** — `POST /auth/register` (DTO с class-validator, хэширование пароля через `argon2`, проверка уникальности email — `409`), опциональное подтверждение email (OTP/magic link) по флагу `AUTH_REGISTER_REQUIRE_EMAIL_CONFIRMATION` — сущность `EmailVerification` (FK на `users`, `ON DELETE CASCADE`), `MailerModule` (nodemailer + Mailpit в docker-compose для дев-окружения), эндпоинты `confirm-otp`/`confirm-link`/`resend`, аудит-логи через `Logger`, юнит-тесты (`auth.service.spec.ts`, `auth.controller.spec.ts`, `email-verification.service.spec.ts`). Токены/сессия при регистрации не выдаются (JWT — T-012). Проверено вручную на живом сервере (Postgres + Mailpit): все 5 acceptance-сценариев из тикета, включая оба метода подтверждения (OTP и magic link). По пути найдено и исправлено несколько багов (проигнорированный результат `canResend()` в `resend()`, неверные HTTP-статусы на `confirm-otp`/`resend`, сравнение Joi-boolean-конфига со строкой `'true'` вместо `String(...)` — подробности в [IMPLEMENTATION-LOG.md](IMPLEMENTATION-LOG.md#t-011-регистрация--фазы-13-конфиг-emailverification-сервисы). Полный контракт, таблица ошибок и зафиксированные архитектурные решения — [docs/tickets/T-010-011-registration.md](tickets/T-010-011-registration.md).
- [ ] **T-012. Аутентификация** — `POST /auth/login`, JWT access + refresh токены, `passport-jwt` стратегия, глобальный guard + `@Public()`, опциональное подтверждение входа через email (переиспользует `EmailVerification` из T-011 с новым полем `purpose`), лимит неудачных попыток / временная блокировка аккаунта. Полный контракт — [docs/tickets/T-012-login.md](tickets/T-012-login.md).
- [ ] **T-013. RBAC** — динамическая модель (роли/разрешения/назначения в БД, admin CRUD API, in-memory кеш с перезагрузкой при мутациях) вместо изначально задуманного статического enum+`RolesGuard` — требования расширены (см. Notion). Жёстко зависит от T-012 (проверка доступа и admin-эндпоинты RBAC берут личность/роль из JWT). Полный контракт — [docs/tickets/T-013-rbac.md](tickets/T-013-rbac.md).
