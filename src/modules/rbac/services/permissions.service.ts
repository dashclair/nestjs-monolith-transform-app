...
export class PermissionsService {
  ...
  async create(
    ...
    const permission = await this.permissionsRepo.save(
      this.permissionsRepo.create(dto),
    );
    await this.rbacConfigService.reload();
    this.logger.log({
      event: 'rbac.permission.created',
      actorUserId,
  ...
  async update(
    ...
    if (dto.name && dto.name !== permission.name) {
      const duplicate = await this.permissionsRepo.findOneBy({
        name: dto.name,
      });
      if (duplicate) {
        throw new ConflictException('Permission with this name already exists');
  ...
  async remove(permissionId: string, actorUserId: string): Promise<void> {
    ...
    const activeGrants = await this.grantsRepo.count({
      where: { permissionId },
    });
    if (activeGrants > 0) {
