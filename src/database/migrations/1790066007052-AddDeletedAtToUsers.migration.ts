...
export class AddDeletedAtToUsers1790066007052 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."email_verifications_purpose_enum" AS ENUM('register', 'login', 'email_change', 'delete_account')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."email_verifications_purpose_enum_old" AS ENUM('register', 'login', 'email_change')`,
    );
    await queryRunner.query(
      `ALTER TABLE "email_verifications" ALTER COLUMN "purpose" DROP DEFAULT`,
    );
